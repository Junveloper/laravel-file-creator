...
export default class NamespaceResolver {
  ...
  private findComposerFile(folder: string): any {
    ...
    const composerFilePath = vscode.workspace
      .getConfiguration("phpCreateClass")
      .get("composerFilePath") as string;
  ...
  private parseComposerFilePath(
    ...
    if (parsedPath.ext === ".json") {
      return {
        composerFolder: this.ensureEndsWithSystemSeparator(
          path.dirname(folder)
        ),
        composerPath: folder,
        composerFound: true,
